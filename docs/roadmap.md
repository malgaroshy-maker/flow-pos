...
- [x] **V1.6.2 (2026-07-23)** — Milestone M closed: Sidebar UX restyle & sample seed data. Prominent logo header added above Dashboard tab in sidebar; redundant Settings tab removed from sidebar (using top header button); user PIN-switcher card compacted; sidebar vertical scrollbar eliminated to show all tabs in a single batch; database seed script (`seed.ts`) populated with rich café & restaurant consumables, equipment with warranties, customers, and suppliers.
